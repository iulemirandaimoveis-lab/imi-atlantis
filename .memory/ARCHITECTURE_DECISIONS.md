...
- **D-16** Contraste do site público corrigido por valor de cor (sem refactor de tokens) + gate `color-contrast` do axe em `e2e/a11y.spec.ts`. O gate **precisa rolar a página toda antes de auditar** (framer-motion `whileInView` deixa tudo abaixo da dobra em `opacity: 0`).
