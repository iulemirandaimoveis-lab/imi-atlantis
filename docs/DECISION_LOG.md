...
## D-16 · 2026-07-31 — Contraste do site público travado por axe no E2E, sem refactor de tokens
- **Contexto**: a mesma classe de bug voltou três vezes (#379 contraste nas páginas escuras, #381 breadcrumb invisível, `--t3` do dark em `globals.css`): cor de um tema aplicada sobre o fundo do outro. Causa estrutural: `[lang]/(website)/layout.tsx` pinta a casca de navy `#0B1928`, enquanto `UI_DESIGN_STANDARDS` §4 declara fundo claro `#F7F5F2` e `globals.css :root` assume creme `#F8F6EE` — três respostas para "qual é o fundo" — e não havia `body { color }`, então texto sem cor explícita virava preto do browser. Nenhum teste cobria contraste (A-04/A-05 do `ACCESSIBILITY_REPORT`).
- **Decisão**: (a) corrigir por **valor de cor**, arquivo a arquivo, sem migrar os ~2.200 hex inline dos 110 arquivos do site público para tokens CSS — refactor de alto risco sob a `UI_REGRESSION_POLICY` e sem ganho visual; (b) definir `body { color: var(--text-primary) }` como rede de segurança; (c) adotar **`@axe-core/playwright`** (devDependency; anti-padrão A9 exige este ADR) e ligar a regra `color-contrast` em `e2e/a11y.spec.ts` sobre 7 rotas públicas.
- **Consequências**: o gate **precisa percorrer a página inteira antes de auditar** — as seções usam framer-motion `whileInView`, então tudo abaixo da dobra fica em `opacity: 0` e o axe (corretamente) ignora elemento invisível; sem o scroll o gate só enxerga a primeira dobra e passa em falso. Overlay decorativo não deve usar `opacity` no ancestral: o axe não resolve isso e acusa falso positivo (foi o caso da faixa dourada em `/pt/credito`, lida como 1,1:1 onde o real é ~7:1) — expressar a opacidade na própria cor (`bg-[#C8A44A]/10`). E2E ainda **não roda no CI** (D-06 de TECH_DEBT), então este gate só protege quem rodar `npm run test:e2e` localmente ou contra preview.
